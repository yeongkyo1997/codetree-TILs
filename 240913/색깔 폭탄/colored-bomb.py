...
def bomb():
        ...
        while q:
            ...
            for dx, dy in dir:
                ...
                if 0 <= nx < N and 0 <= ny < N and board[nx][ny] != -1:
                    # 빨강색이라면 visited를 신경안쓰고 red_visited를 이용해야함
                    if board[nx][ny] == -2:
                        if (nx, ny) in red_visited:
                            continue
    ...
    for i in range(N - 1, -1, -1):
        for j in range(N):
            if (board[i][j] > 0  # 빨간으로만 이루어진 경우 폭탄 묶음이 될 수 없음
                    and not visited[i][j]):
                cnt, red_cnt = bfs(i, j)
                # 폭탄 묶음의 크기가 2보다 크다면 후보가 될 수 있음
                if cnt >= 2:
    ...
    while q:
        ...
        for dx, dy in dir:
            ...
            if (0 <= nx < N
                    ...
                    and (board[nx][ny] == -2 or board[nx][ny] == board[x][y])):
                q.append((nx, ny))
                visited[nx][ny] = True
                path.append((nx, ny))
